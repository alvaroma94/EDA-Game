...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   //SOLDADOS
	...
	while(not q.empty()) {
		...
		if(valid(xx,yy) and not visitados[xx][yy] and que(xx,yy) < 3 and temps_foc(xx,yy) == 0 and ((quin_soldat(xx,yy) == 0) or (dades(quin_soldat(xx,yy)).equip != mi_equipo))) {
  ...
  virtual void play () {
	...
	if (not dades(H[1]).paraca.empty()) caure_paracas(H[1]);
